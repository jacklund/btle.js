...
function Service(declaration, includes, characteristics, endHandle, peripheral) {
  ...
  if (endHandle) {
    ...
    if (characteristics && characteristics.length > 0) {
      this.endHandle = characteristics[characteristics.length-1].declaration.endHandle;
    } else if (includes && includes.length > 0) {
      this.endHandle = includes[includes.length-1].handle;
    } else {
      this.endHandle = declaration.handle;
    }
  }
  this.declaration.endHandle = this.endHandle;
