...
Connection.prototype.findService = function(type, callback) {
  ...
  if (type == null) {
    this.readByGroupType(0x001, 0xFFFF, gatt.Attributes.PRIMARY_SERVICE_UUID.toString(16), function(err, list) {
      if (err) {
        callback(err, null);
      } else {
    ...
    this.findByTypeValue(0x001, 0xFFFF, gatt.Attributes.PRIMARY_SERVICE_UUID.toString(16),
		    stringToUuid(type), function(err, list) {
      if (err) {
        callback(err, null);
      } else {
        var ret = new Array(list.length);
