...
// Connect to a device
module.exports.connect = function(destination, callback) {
  btle.connect(destination, function(err, connection) {
    ...
    // Get the device name
    device.findService(gatt.Services.GAP, function(err, list) {
      if (err) {
        callback(err, null);
      } else {
        list[0].findCharacteristics(gatt.CharTypes.DEVICE_NAME, function(err, list) {
