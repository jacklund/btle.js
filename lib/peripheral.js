...
function handleError(peripheral, central, data) {
  console.log("Error response, opcode = %d, handle = %d, error code = %d",
...
function handleMTU(peripheral, central, data) {
  ...
  console.log("Requested MTU = %d, my MTU = %d", requestedMTU, this.mtu);
...
function handleFindInfo(peripheral, central, data) {
  ...
  console.log("Find Info, start=%d, end=%d", start, end);
}
...
function handleFindByType(peripheral, central, data) {
  ...
  console.log("Find by Type Value, start=%d, end=%d, type=%s, value=%s",
...
function handleReadByGroup(peripheral, central, data) {
  ...
  console.log("Read by Group Type, start = %d, end = %d, type = %s", start, end, type);
  var attributes = peripheral.types[type];
  var response = new Buffer(peripheral.mtu);
  if (attributes && attributes.length > 0) {
    response[0] = att.Opcodes.READ_BY_GROUP_RESPONSE;
    response[1] = attributes[0].value.length;
    var offset = 2;
...
Peripheral.prototype.listen = function(opts, callback) {
  ...
  central.listen(opts, function(err, c) {
    ...
    if (c.listeners('data').length == 0) {
      console.log('registering data handler');
      c.on('data', function(data) {
        console.log('data handler called');
        var opcode = data[0];
        console.log('opcode = %d', opcode);
