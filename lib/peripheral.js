...
function Peripheral(name, options, services) {
  ...
  if (services.length < 2 || services[1].uuid != gatt.Services.GATT) {
    console.log("Creating a GATT service");
  ...
  if (services.length == 0 || services[0].uuid != gatt.Services.GAP) {
    console.log("Creating a GAP service");
  ...
  console.log("Have %d services", services.length);
  ...
  var handle = 1;
  for (var i = 0; i < services.length; ++i) {
    console.log('Setting handle for service %d to %d', i, handle);
  ...
  console.log("Have %d attributes", this.attributes.length);
...
function sendAttributeNotFound(central, op, handle) {
  console.log('sendAttributeNotFound, op = %d, handle = %d', op, handle);
  var response = new Buffer(5);
  response[0] = att.Opcodes.ERROR;
  response[1] = op;
...
function handleReadByType(peripheral, central, data) {
  ...
  if (attributes.length == 0)
    return sendAttributeNotFound(central, att.Opcodes.READ_BY_TYPE_REQUEST, start);
...
Peripheral.prototype.listen = function(opts) {
  console.log('listen, opts = %s', opts);
  var peripheral = this;
  if (!opts) opts = {};
