...
Peripheral.prototype.on = function(event, callback) {
  this.central.on(event, callback);
}

// Send an Invalid Handle error
function sendInvalidHandle(central, op, handle) {
  var response = new Buffer(5);
  response[0] = att.Opcodes.ERROR;
  response[1] = op;
  response[2] = 0;
  response[3] = handle;
  response[4] = att.ErrorCodes.INVALID_HANDLE;
  central.write(response);
}

// Send an Attribute Not Found error
function sendAttributeNotFound(central, op, handle) {
  var response = new Buffer(5);
  response[0] = att.Opcodes.ERROR;
  response[1] = op;
  response[2] = 0;
...
function findAttributes(handles, start, end, type, value) {
  ...
  for (var i = start; i < end; ++i) {
    if (handles[i] && (!type || handles[i].uuid == type) &&
...
function handleFindInfo(peripheral, central, data) {
  ...
  if (start == 0x0000 || start > end)
    return sendInvalidHandle(central, att.Opcodes.FIND_INFO_REQUEST, start);
  ...
  if (attributes.length == 0)
    return sendAttributeNotFound(central, att.Opcodes.FIND_INFO_REQUEST, start);
...
function handleFindByType(peripheral, central, data) {
  ...
  // Validate args
  if (start == 0x0000 || start > end)
    return sendInvalidHandle(central, att.Opcodes.FIND_BY_TYPE_REQUEST, start);
  ...
  if (attributes.length == 0)
    return sendAttributeNotFound(central, att.Opcodes.FIND_BY_TYPE_REQUEST, start);
...
function handleReadByType(peripheral, central, data) {
  ...
  // Find the attributes
  var attributes = getAttributes(peripheral.handles, start, end, type);

  // If no matches, send Attribute Not Found
  if (attributes.length == 0)
    return sendAttributeNotFound(central, att.Opcode.READ_BY_TYPE, start);
...
function handleRead(peripheral, central, data) {
  ...
  // No attribute found
  if (!attrib) return sendAttributeNotFound(central, att.Opcodes.READ_REQUEST, handle);
...
function handleReadByGroup(peripheral, central, data) {
  ...
  // No attributes
  if (attributes.length == 0) return sendAttributeNotFound(central, att.Opcodes.READ_BY_GROUP_REQUEST, start);
...
function handleWrite(peripheral, central, data) {
  ...
  if (btle.debug()) console.log("Write, handle = %s, end = %d, type = %s", start, end, type);
}
...
Peripheral.prototype.listen = function(opts, callback) {
  console.log('listen, opts = %s, callback = %s', opts, callback);
  var peripheral = this;
  ...
  this.central.listen(opts, function(err, c) {
    console.log("listen callback, err = %s, c = %s", err, c);
    if (err) {
    ...
    console.log(c.listeners('data').length);
