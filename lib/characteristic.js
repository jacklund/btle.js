...
function Characteristic(handle, properties, uuid, value, descriptors) {
  ...
  // The declaration
  var declaration = att.createAttribute(handle, gatt.AttributeTypes.CHARACTERISTIC, declValue, this.endHandle);
  declaration.properties = Properties.READ;
  this.attributes.push(declaration);

  // The value
  var valAttr = att.createAttribute(handle && handle+1, uuid, value);
  valAttr.properties = properties;
  valAttr.on('valueChanged', function(attrib) {
    this.emit('valueChanged', attrib);
  });
